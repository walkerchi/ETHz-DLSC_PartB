...
class MeshNeuralOperatorTrainer(TrainerBase):
    ...
    def __init__(self, config):
        ...
        self.weight_path       = f"weights/{config.equation}_{'_'.join([f'{k}={v}' for k,v in equation_kwargs.items()])}/spatial={self.config.n_train_spatial}/{config.model}"
        self.image_path        = f"images/{config.equation}_{'_'.join([f'{k}={v}' for k,v in equation_kwargs.items()])}/spatial={self.config.n_train_spatial}/{config.model}"
